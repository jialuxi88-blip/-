...
1.准备两种数据集图片，测试集每份120-160张左右，验证集每份60张左右，图片的文件目录是上传的dataset文件夹目录，不要更改位置和文件名！！！不要更改
...
在运行代码.py文件中第50行# 标签映射中↑，替换自己分类的类型，'classical_image'和'morden_image'都必须替换成自己想要分类的名字，注意是在代码中所有出现的这两种字符都要替换！！
